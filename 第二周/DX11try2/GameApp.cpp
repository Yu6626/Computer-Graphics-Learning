...
// 需要整理一下整个流程
// 有无ImGui会影响这个函数内部的算法,也与着色器的数据设置有关，但它们之间联系说实话，没太懂
void GameApp::UpdateScene(float dt)
{
    ...
    // 自定义窗口与操作
    //
    static float tx = 0.0f, ty = 0.0f, phi = 0.0f, theta = 0.0f, scale = 1.0f, fov = XM_PIDIV2; // 相关参数的设置
    static bool animateSquareFrustum = true, customColor = false;
    // 控制投影变换方式,切换为正交投影
    static float orthoWidth = 18.0f;
    static bool useOrtho = false;
    if (animateSquareFrustum) 
    {   // 其实也是在一个大循环里面，每帧每帧地更新物体位置数据，从而让物体动起来
    ...
    {
        ImGui::Checkbox("Animate Square Frustum", &animateSquareFrustum); // 控制是否旋转
        ...
        ImGui::SliderFloat("Scale", &scale, 0.2f, 0.2f);
        ...
        {
            ImGui::Text("orthoWidth: %.2f ", orthoWidth);
            ImGui::SliderFloat("##3", &orthoWidth, 9.0f, 40.0f); // 注意格式
        }

        if (ImGui::Checkbox("Use Custom Color", &customColor))
            m_CBuffer.useCustomColor = customColor; // 需要着色器的设置改变，所以特殊处理
        if (customColor)
        {
    ...
    {
        m_CBuffer.proj = XMMatrixTranspose(XMMatrixOrthographicLH(orthoWidth, orthoWidth / AspectRatio(), 1.0f, 1000.0f));
    }
    // 更新常量缓冲区
...
bool GameApp::InitEffect()
{
    ...
    // 创建顶点着色器
    HR(CreateShaderFromFile(L"HLSL\\Square_frustum_VS.cso", L"HLSL\\Square_frustum_VS.hlsl", "VS", "vs_5_0", blob.ReleaseAndGetAddressOf()));
    ...
    // 创建像素着色器
    HR(CreateShaderFromFile(L"HLSL\\Square_frustum_PS.cso", L"HLSL\\Square_frustum_PS.hlsl", "PS", "ps_5_0", blob.ReleaseAndGetAddressOf()));
...
{
    // ******************
    // 设置立方体顶点 -> 四棱台的顶点 请注意在DX11中，采用的是左手坐标系，即X右，Y上，Z前
    //    5________ 6
    //    /|      /|
    //   /_|_____/ |
    //  1|4|_ _ 2|_|7
    ...
    // 索引数组
    //
    DWORD indices[] = { // 从左下角开始构造，沿顺时针的方向
        // 正面  
        0, 1, 2,
        2, 3, 0,
        // 左面
    ...
    m_CBuffer.view = XMMatrixTranspose(XMMatrixLookAtLH( // 视图矩阵
    ...
    D3D11SetDebugObjectName(m_pVertexShader.Get(), "Square_frustum_VS");
    D3D11SetDebugObjectName(m_pPixelShader.Get(), "Square_frustum_PS");
